...
--
-- ┌──────────────────────────────────────────────────────────────────────────┐
-- │ 2026-08-07 현재 이 파일은 이미 반영된 상태다. 다시 돌려도 0건이다.        │
-- │                                                                          │
-- │ 앞서 크래시한 통합 스크립트가 튕기기 전에 여기 [B-2] 와 같은 UPDATE 를    │
-- │ 실행했다. 그래서 id 50 · 104 가 전환됐다. 규칙은 의도한 것과 같지만       │
-- │ 확인 없이 나간 쓰기이므로 D 파일 D-1 로 사후 검증한다.                    │
-- │                                                                          │
-- │ 남은 4개(5 · 54 · 71 · 74)는 전부 이 규칙에 안 걸린다 —                   │
-- │ 5 · 74 는 제외 대상이고, 54 · 71 은 카테고리있는그룹수가 2다.             │
-- │ 그래도 B-1 을 돌려 0건인지 확인해 두면 확실하다(읽기 전용).               │
-- └──────────────────────────────────────────────────────────────────────────┘
--
